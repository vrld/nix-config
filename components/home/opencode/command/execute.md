...
- Run the success criteria checks (linter and type checker cover most of it)
...
Use the jujutsu version control system to structure your changes:
- Before each phase, create a new `jj` change with `jj new -m "<one-line description in conventional commit format>"`
- Implement the changes
- Update the description with more details: `echo "description with multiple lines" | jj describe --stdin`. Keep the original conventional commit description at the top.
...
4. **Implement each phase sequentially**, adapting to what you find while following the plan's intent and creating `jj` changes for each phase.
